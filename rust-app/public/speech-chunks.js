export class SpeechChunks {
  ...
  push(pcm, timing) {
    ...
    for (let start = 0; start < pcm.length; start += 160) this.frame(pcm.subarray(start, start + 160), timing ? timing.start + start / 16000 : undefined, timing?.endOfMedia === true);
  }
  frame(pcm, time, endOfMedia = false) {
    let energy = 0;
    ...
    if (this.silence >= 16000 * 0.4 || this.offset >= this.samples.length) {
      if (this.offset - this.silence >= 16000 * 0.25) this.send(true, endOfMedia);
      this.reset();
    } else if (this.maxSeconds === 16 && this.offset >= 16000 && this.sinceUpdate >= 16000 * 0.5) this.send(false, endOfMedia);
  }
}

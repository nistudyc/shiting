...
3. 打开 YouTube 视频或直播，点击扩展图标，粘贴配对码，选择翻译方式，点击「开启当前视频字幕」。
4. 首次使用等待桌面端下载模型；进度显示在弹窗和视频中。开始播放英语视频后显示两行字幕。点击「停止」结束捕获。

Google / 火山密钥在桌面端配置。切换翻译方式须先停止，再开启。YouTube 站内跳转（含进入或切换直播）不影响捕获，字幕跟随当前标签页；整页刷新、离开 YouTube 或关闭标签页时停止，重新点击开启。全屏字幕跟随 YouTube 播放器。字幕为实时识别结果，存在识别/翻译延迟，不保证逐字准确。

...
3. 用户已明确授权在本机钥匙串创建更新签名密钥；密钥已创建，仅公钥进入源码。已完成应用、DMG、更新归档与 appcast 签名检查；安全审查补充严格验签配置后重新打包。
...
Sparkle 更新归档解压前验签，更新清单也要求签名；SUSignedFeedFailureExpirationInterval 固定为0，避免长时间验签失败后宽限处理。最终发布与下载回验记录位于工作区 .omo/evidence/release-checks-20260920.md。
